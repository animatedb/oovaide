...
void DiagramDependencyDrawer::drawArrowDependency(GraphPoint &consumerPoint,
        GraphPoint &supplierPoint)
    {
    mDrawer->drawLine(supplierPoint, consumerPoint);
    DiagramArrow arrow(supplierPoint, consumerPoint, 10);
    mDrawer->drawLine(supplierPoint, supplierPoint + arrow.getLeftArrowPosition());
    mDrawer->drawLine(supplierPoint, supplierPoint + arrow.getRightArrowPosition());
    }

GraphRect DiagramDependencyDrawer::getNodeRect(size_t nodeIndex) const
    {
    ...
    float textHeight = mDrawer->getTextExtentHeight("Wy");
...
size_t DiagramDependencyDrawer::getNodeIndex(GraphPoint p, size_t numNodes) const
    {
