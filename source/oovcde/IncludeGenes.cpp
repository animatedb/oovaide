...
void IncludeGenes::initialize(IncludeDrawer &drawer, size_t nodeHeight)
    {
    mDrawer = &drawer;
...
size_t IncludeGenes::getMaxNodesInColumn() const
    {
...
void IncludeGenes::updatePositionsInDrawer()
    {
...
void IncludeGenes::setupQualityEachGeneration()
    {
    mMaxDistanceQ = 0;
    mMaxOverlapQ = 0;
    mMaxHeightQ = 0;
...
QualityType IncludeGenes::calculateSingleGeneQuality(size_t geneIndex) const
    {
...
size_t IncludeGenes::getNodeYDistances(size_t geneIndex) const
    {
    size_t distance = 0;
    for(size_t ci=0; ci<mDrawer->getNumConnections(); ci++)
	{
	IncludeConnection const &conn = mDrawer->getConnection(ci);
...
size_t IncludeGenes::getNodeOverlapCount(size_t geneIndex) const
    {
...
bool IncludeGenes::nodesOverlap(size_t geneIndex, size_t node1, size_t node2) const
    {
...
GeneValue IncludeGenes::getYPosition(size_t geneIndex, size_t nodeIndex) const
    {
    return getValue(geneIndex, nodeIndex*sizeof(GeneValue));
    }

void IncludeGenes::setYPosition(size_t geneIndex, size_t nodeIndex, GeneValue val)
    {
    return setValue(geneIndex, nodeIndex*sizeof(GeneValue), val);
    }

size_t IncludeGenes::getDrawingHeight(size_t geneIndex) const
    {
    size_t ySize = 0;
